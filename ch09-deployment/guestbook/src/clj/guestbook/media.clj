...
(defn insert-image-returning-name
  ...
  (let [baos (ByteArrayOutputStream.)
        ...
        ratio (cond
                ...
                (some? height) (/ height img-height)

                (some? width) (/ width img-width)

                (and (some? max-height) (some? max-width))
                (min 1 (/ img-width max-width) (/ img-height max-height))

                (some? max-height) (min 1 (/ img-height max-height))

                (some? max-width) (min 1 (/ img-width max-width))
    ...
    (if (= 0
           (db/save-file! {:name filename
                           ...
                           :type "image/png"}))
      (do
        (log/error "Attempted to overwrite an image you dont own")
        (throw (ex-info "Attempted to overwrite an image you dont own" {:name filename})))
      filename)))

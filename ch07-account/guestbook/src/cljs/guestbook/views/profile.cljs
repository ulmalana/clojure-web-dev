...
(rf/reg-sub
 ...
 :<- [:profile/media]
 (fn [[changes media] _]
   (not (and (empty? changes) (empty? media)))))
     ...
     {:ajax/upload-media!
      {:url "/api/my-account/media/upload"
       ...
       (fn [response]
         (rf/dispatch
          [:profile/set-profile
           (merge profile (select-keys response (:files-uploaded response)))]))}}
                ...
                (if (not= new-password confirm-password)
                  ...
                  (ajax/POST "/api/my-account/change-password"
                             {:params params
                              :handler
                              (fn [_]
                                ;;display success message for 5 s
                                (swap! success
                                       (fn [{:keys [timeout]}]
                                         (when timeout
     ...
     [m/modal-button
      ...
      [:section
       [:div.message.is-danger
        [:div.message-header (str "deleteing account " username)]
